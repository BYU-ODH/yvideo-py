		...
		get: async () => {
			try {
				if (window.clj_session_id === `{{ session-id }}`) {
					...
					await axios.get(`${process.env.REACT_APP_YVIDEO_SERVER}/api/get-session-id/${process.env.REACT_APP_USER_NAME}/${process.env.REACT_APP_USER_KEY}/${process.env.REACT_APP_USER_KEY_BYUID}/asdf`).then(async res => {
