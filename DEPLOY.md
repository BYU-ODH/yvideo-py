...
- **Legacy migration SSH access** (temporary — see `core/legacy_migration/REMOVAL.md`): When `LEGACY_MIGRATION_MEDIA_ROOT` in `secret_settings.py` points at a remote `host:/path`, `core.legacy_migration` shells out to `ssh`/`scp` inside the container. The Quadlet mounts the deploy user's own `~/.ssh` read-only into the container at `/root/.ssh`, so the deploy user must already have a working, passwordless (`BatchMode=yes`) SSH key and a `known_hosts` entry for that host — verify with `ssh -oBatchMode=yes <host> true` as the deploy user before relying on legacy migration. This bullet, the `openssh-client` install in the `Dockerfile`, and the `~/.ssh` mount all go away together once legacy_migration is removed.
