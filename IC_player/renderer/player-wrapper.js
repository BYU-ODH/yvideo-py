...
export const player = {
  ...
  initializeOrSelectFiles: () => {
    ...
    if (!files) {
      ...
      function onFileChange() {
        ...
        document.getElementById('filePicker').removeEventListener('change', onFileChange)
        // Add play button event listener here
        document.getElementById('playButton').onclick = () => {
          player.startPlayer()
        }
      }
      document.getElementById('filePicker').addEventListener('change', onFileChange)
    } else {
      player.startPlayer()
    }
  },
  ...
  hidePlayer: () => {
    ...
    document.getElementById('playButton').onclick = () => player.initializeOrSelectFiles()
    document.getElementById('files').textContent = 'Select Files'

    // Reset annotation mode state
    player.annotationMode = false
    document.getElementById('toggleAnnotationModeBtn').classList.remove('active')
  ...
  getSelectedFiles: () => {
    ...
    if (icfFileExists && (!jsonFileExists || !videoFileExists)) {
      ...
      const jsonPath = webUtils.getPathForFile(icfFile).replace(/\/[^\/]*$/, '/' + icfObj['annotation'])
      const videoPath = webUtils.getPathForFile(icfFile).replace(/\/[^\/]*$/, '/.ic/' + icfObj['video'])
  ...
  initializePlayerAndPlay: (fileData) => {
    ...
    if (!player.annotationPlayer) {
      player.annotationPlayer = new AnnotationPlayer(videoElem, annotationContainer)
    }
    // Defensive: parse fileData if it's a Buffer or string
    let parsedData = fileData
    if (fileData && (fileData instanceof Buffer || typeof fileData === 'string')) {
      try {
