export class AnnotationPlayer {
    ...
    loadAnnotations(annotationData) {
        this.annotations = [];
        const jsonObj = typeof annotationData === 'string' ? JSON.parse(annotationData) : annotationData;
        if (jsonObj['media']) {
        ...
        } else if (jsonObj[0]['type'] && jsonObj[0]['start'] && jsonObj[0]['end']) { // TODO ensure this is robust
        ...
        for (let i = 0; i < this.annotations.length; i++) {
            if (this.annotations[i].type === 'censor') {
                ...
                censor[2] = [];
                Object.entries(this.annotations[i].details.position).forEach(([key, val]) => {
                ...
                case 'censor':
                    if (time >= aStart && time < aEnd) {
                        if (!this.annotationContainer.querySelector('#censor' + i)) {
                            ...
                            const censor = this.annotationContainer.querySelector('#censor' + i);
                            let annoTime;
                            if (a.details.interpolate) {
                                annoTime = Object.keys(a.details.intPositions).reduce((prev, curr) =>
                                ...
                                annoTime = Object.keys(a.details.position).reduce((prev, curr) =>
                                    Math.abs(curr - time) < Math.abs(prev - time) ? curr : prev
                                );
                        ...
                        const existingCensor = this.annotationContainer.querySelector('#censor' + i);
                        if (existingCensor) {
    ...
    resetAnnotations() {
        ...
        Array.from(this.annotationContainer.querySelectorAll('[id^=censor]')).forEach(el => el.remove());
        this.unmute();
    }
    ...
    interpolateCensor(annotation) {
        ...
        let timeKeys = Object.keys(position).sort((a, b) => parseFloat(a) - parseFloat(b));
        for (let i = 0; i < timeKeys.length; i++) {
            ...
            let wincr = null, hincr = null;
            if (position[t1][2] && position[t1][3] && position[t2][2] && position[t2][3]) {
