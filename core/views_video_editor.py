...
# TODO move this into video editor view
def clip_editor(request, content_id):
    ...
    context = {
        "content": content,
        "file_key": file_key.id if file_key else None,  # is this needed?
        "clips": clips,
    }
...
@require_GET
def load_clip_form(request, content_id, clip_id):
    try:
        content = Content.objects.get(pk=content_id)
        clip = Clip.objects.get(pk=clip_id)
    except Content.DoesNotExist:
        logger.error("Failed to load clip form because the content does not exist")
        return HttpResponseBadRequest()
    except Clip.DoesNotExist:
        logger.error("Failed to load clip form because it doesn't exist")
        return HttpResponseBadRequest()
    except Exception as e:
        logger.error(f"Failed to load clip form. Exception: {e}")
        return HttpResponseServerError()
    ...
    # Restart here, check what partial we should be sending back to client and
    # what data requirements it has
